...
# =====================================================================
VIDEO_TITLE = "Ranking Awesomely Entertaining Trickshots"
CLIP_DURATION = 12
...
VIDEO_SOURCES = [
    # TikTok videos
    {"url": "https://www.tiktok.com/@espn/video/7667956168039484702?is_from_webapp=1&sender_device=pc&web_id=7673309675748886029", "start": 0, "end": 7},
    {"url": "https://www.tiktok.com/@espn/video/7676615217605807390?is_from_webapp=1&sender_device=pc&web_id=7673309675748886029", "start": 0, "end": 13},
    {"url": "https://www.tiktok.com/@thatsinsanne/video/7254681756765326618?is_from_webapp=1&sender_device=pc&web_id=7673309675748886029", "start": 0, "end": 14},
    {"url": "https://www.tiktok.com/@matthewsadler__/video/7647228261486939422?is_from_webapp=1&sender_device=pc&web_id=7673309675748886029", "start": 0, "end": 5},
    {"url": "https://www.tiktok.com/@espn/video/7673662499396406558?is_from_webapp=1&sender_device=pc&web_id=7673309675748886029", "start": 0, "end": 8},
    #{"url": "https://www.tiktok.com/@tristanjamesofficial/video/7671548093401206046?is_from_webapp=1&sender_device=pc&web_id=7673309675748886029", "start": 0, "end": 6},
...
def download_tiktok_video(url):
    """Download a TikTok video natively by structuring valid extractor contexts."""
    ydl_opts = {
        ...
        # Bypasses programmatic AssertionError check loops completely
        'compat_opts': {'no-impersonate-check'},
        
        # FIX: Passes the impersonation context directly into the extractor engine
        # safely establishing matching network handshakes across link cycles.
        'extractor_args': {
            'tiktok': {
                'impersonate': 'chrome',
            }
    ...
    with YoutubeDL(ydl_opts) as ydl:
        try:
            ...
            if not info:
                print("   ⚠️ TikTok returned empty payload. Session regeneration required.")
                return None
...
def process_single_video(source, index, total_clips, processed_clips):
    ...
    # Determine if it's a local file or TikTok URL
    if "local" in source:
    ...
    try:
        ...
        # --- FIX: Calculate absolute numbers for position instead of using 'center' ---
        # This prevents MoviePy v2.x from crushing the layout canvas boundaries.
...
##
        ...
        for r in range(total_clips, 0, -1):
            ...
            # --- FIX: Shrink box widths to lock them into the left sidebar column ---
            # This allows padding for the thick strokes while forcing the text to stay left
            ...
            # --- Position the Clips ---
            # Precise layout coordinates to keep everything perfectly flush
            label_offset = 60 if is_active else 35  #was 110 and 75
        ...
        ##    # --- 1. Generate Your Standard Text Clips Natively ---
        ##    # Pad text with trailing spaces to prevent ImageMagick boundary clipping
        ##    display_label = f"  {label_text}     "
        ##    display_num = f"  {r}.   "
##
        ##    # Calculate width boundaries based on master resolution rules
        ##    max_text_width = MASTER_WIDTH - LIST_PADDING_LEFT - 100
##
        ##    num_clip = TextClip(
        ##        text=display_num, font=FONT_PATH, font_size=text_size, color=num_color,
        ##        stroke_color='black', stroke_width=stroke_w, method="label"
        ##    )
        ##    
        ##    # FIXED: text_align set to "left" to avoid pipeline crash
        ##    label_clip = TextClip(
        ##        text=display_label, font=FONT_PATH, font_size=text_size, color=label_color,
        ##        stroke_color='black', stroke_width=stroke_w, method="caption",
        ##        size=(max_text_width, None), text_align="left"
        ##    )
        ##    # --- 2. Calculate the Vertical Position ---
        ##    y_pos = LIST_START_Y + ((total_clips - r) * LIST_SPACING_Y)
        ##    
        ##    # --- 3. Position the Clips Natively ---
        ##    # Using your original, working side-by-side coordinates. No offsets, no shifts.
        ##    num_clip = num_clip.with_position((LIST_PADDING_LEFT, y_pos)).with_duration(trimmed_clip.duration)
        ##    label_clip = label_clip.with_position((LIST_PADDING_LEFT + 70, y_pos)).with_duration(trimmed_clip.duration)
        ##    
        ##    layers.append(num_clip)
        ...
        # Clean up temporary TikTok downloads
        if "url" in source and os.path.exists(file_path):
            try:
                os.remove(file_path)
            except:
                pass  # Will be cleaned up later if needed
...
def main():
    ...
    if processed_clips:
        ...
        music_audio_path = "The_Iron_Maw.mp3"
